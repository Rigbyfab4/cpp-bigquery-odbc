...
and tests using Bazel. The intended audience is developers of the
driver who want to verify their changes will work with bazel.
...
Bazel runs builds in a sandbox, so it cannot automatically have access to the env variables. We will have to pass
the through `--test_env` for `bazel test` or `bazel run`.
...
You can run the client library integration tests one by one. Here is an example of running the 
`Get Dataset` test:
...
(TODO: [#61](https://github.com/googleapis/cpp-bigquery-odbc/issues/61))
You can also run all the tests in one go:
