...
// is returned verbatim (right-trimmed). Otherwise it is an ODBC LIKE pattern and
// is a literal only when it has no unescaped '%'/'_' metacharacter, in which
// case the unescaped form is returned. Lets SQLTables skip a projects.list /
// datasets.list round trip when a concrete project or dataset name was passed.
// Empty patterns return nullopt to preserve the (match-nothing) listing path.
