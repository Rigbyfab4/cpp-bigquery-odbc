...
odbc_internal::StatusRecord ConvertFromTimestampDSValue(
  ...
  switch (dest_type) {
    ...
    case SQL_C_WCHAR: {
      ...
      std::wstring const& timestamp_wstr = wstr.GetValue();
      SQLLEN wchar_capacity = buffer_length / static_cast<SQLLEN>(wire_sz);
      if (wchar_capacity > k_timestamp_src_len) {
        if (res_len) {
          *res_len = static_cast<SQLLEN>(timestamp_wstr.size() * wire_sz);
        }
        WriteWideToWireBuffer(timestamp_wstr, dest_buf,
                              (timestamp_wstr.size() + 1) * wire_sz);
        ...
        WriteWideToWireBuffer(timestamp_wstr, dest_buf,
                              static_cast<size_t>(wchar_capacity) * wire_sz);
        LOG(WARNING)
...
odbc_internal::StatusRecord ConvertFromDatetimeDSValue(DSValue const& src_dsval,
  ...
  switch (dest_type) {
    ...
    case SQL_C_WCHAR: {
      ...
      size_t const wire_sz = WireWcharSize();
      std::wstring const& datetime_wstr = wstr.GetValue();
      SQLLEN wchar_capacity = buffer_length / static_cast<SQLLEN>(wire_sz);
      if (wchar_capacity > k_datetime_src_len) {
        if (res_len) {
          *res_len = static_cast<SQLLEN>(datetime_wstr.size() * wire_sz);
        }
        WriteWideToWireBuffer(datetime_wstr, dest_buf,
                              (datetime_wstr.size() + 1) * wire_sz);
...
StatusRecord ConvertBytesToWChar(DSValue const& conn_val,
  ...
  size_t const wire_sz = WireWcharSize();
  size_t const required_size = utf16_value.size() * wire_sz;
  ...
  if (static_cast<size_t>(dest_data.buflen) >= required_size + wire_sz) {
    WriteWideToWireBuffer(utf16_value, dest_data.buf, required_size + wire_sz);
  } else {
    // Exact fit: write all characters in wire format without null terminator.
    if (IsRuntimeWireUtf16Le()) {
      auto* d = static_cast<uint16_t*>(dest_data.buf);
      for (size_t i = 0; i < utf16_value.size(); ++i)
        d[i] = static_cast<uint16_t>(utf16_value[i]);
    } else {
      auto* d = static_cast<SQLWCHAR*>(dest_data.buf);
      for (size_t i = 0; i < utf16_value.size(); ++i)
        d[i] = static_cast<SQLWCHAR>(utf16_value[i]);
    }
  }
