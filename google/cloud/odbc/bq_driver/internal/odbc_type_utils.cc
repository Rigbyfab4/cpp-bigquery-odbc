...
odbc_internal::StatusRecord WStrIntervalBufferResponse(
  ...
  auto u_char_len = static_cast<size_t>(char_len);
  if (buffer_length > char_len) {
    if (res_len) {
      *res_len =
          static_cast<SQLLEN>(u_char_len) * static_cast<SQLLEN>(wire_sz);
    }
    WriteWideToWireBuffer(wstr, dest_buf, (u_char_len + 1) * wire_sz);
