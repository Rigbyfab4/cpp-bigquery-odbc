...
TEST(StatementTest, SQLDescribeCol_AfterQuery) {
  std::cout << "--- Running TEST(StatementTest, SQLDescribeCol_AfterQuery) ---"
            << std::endl;
  std::string const table_name =
      kDatasetWithTablePrefix + "ODBC_SQLDESCRIBECOL_METADATA_TEST";
  Table table(table_name);
  std::string const schema_name = "INTEGRATION_TESTS";
  std::string const schema_str =
      "(StringField STRING(50), IntField INT64, FloatField FLOAT64, "
      "NumericField NUMERIC(10, 2))";

  // Create Table
  auto conn = std::make_shared<ODBCHandles>();
  std::cout << "Connecting to create table..." << std::endl;
  EXPECT_EQ(Connect(kDefaultConnectionString, conn), SQL_SUCCESS);
  std::cout << "Creating table: " << table_name << " with schema " << schema_str
            << std::endl;
  table.CreateWithPrepare(conn, schema_str);
  EXPECT_EQ(Disconnect(conn), SQL_SUCCESS);

  // Connect and call SQLPrepare/SQLExecute
  std::cout << "Connecting to call SQLPrepare/SQLExecute..." << std::endl;
  EXPECT_EQ(Connect(kDefaultConnectionString, conn), SQL_SUCCESS);
  std::string query = "SELECT * FROM " + table_name;
  std::cout << "Preparing query: " << query << std::endl;
  SQLRETURN ret =
      SQLPrepare(conn->hstmt, (SQLCHAR*)query.c_str(), SQL_NTS);
  CheckError(ret, "SQLPrepare", conn);

  std::cout << "Executing query..." << std::endl;
  ret = SQLExecute(conn->hstmt);
  CheckError(ret, "SQLExecute", conn);

  // 1. Check column count
  SQLSMALLINT col_count = 0;
  ret = SQLNumResultCols(conn->hstmt, &col_count);
  CheckError(ret, "SQLNumResultCols", conn);
  std::cout << "SQLNumResultCols returned: " << col_count << " columns."
            << std::endl;
  ASSERT_EQ(col_count, 4);

  // 2. Define expected metadata
  struct ExpectedCol {
    std::string name;
    SQLSMALLINT type;
    SQLULEN size;
  ...
  std::vector<ExpectedCol> expected_cols = {
      {"StringField", SQL_WVARCHAR, 50, 0, SQL_NULLABLE},
      {"IntField", SQL_BIGINT, 19, 0, SQL_NULLABLE},
      {"FloatField", SQL_DOUBLE, 15, 0, SQL_NULLABLE},
      {"NumericField", SQL_NUMERIC, 10, 2, SQL_NULLABLE}};

  // 3. Describe and validate each column
  std::cout << "Describing columns..." << std::endl;
  for (SQLSMALLINT i = 1; i <= col_count; i++) {
    SQLCHAR col_name[256];
    SQLSMALLINT name_len = 0, data_type = 0, decimal_digits = 0, nullable = 0;
    SQLULEN col_size = 0;
    ...
    ret = SQLDescribeCol(conn->hstmt, i, col_name, sizeof(col_name), &name_len,
                         &data_type, &col_size, &decimal_digits, &nullable);
    CheckError(ret, "SQLDescribeCol", conn);

    std::cout << "  Column " << i << ":" << std::endl;
    std::cout << "    Name: " << col_name << " (Expected: " << expected_cols[i - 1].name << ")" << std::endl;
    std::cout << "    Type: " << data_type << " (Expected: " << expected_cols[i - 1].type << ")" << std::endl;
    std::cout << "    Size: " << col_size << " (Expected: " << expected_cols[i - 1].size << ")" << std::endl;
    std::cout << "    Decimals: " << decimal_digits << " (Expected: " << expected_cols[i - 1].decimals << ")" << std::endl;
    std::cout << "    Nullable: " << nullable << " (Expected: " << expected_cols[i - 1].nullable << ")" << std::endl;

    ExpectedCol& expected = expected_cols[i - 1];
    EXPECT_STREQ((char*)col_name, expected.name.c_str());
    EXPECT_EQ(data_type, expected.type);
    EXPECT_EQ(col_size, expected.size);
    EXPECT_EQ(decimal_digits, expected.decimals);
    EXPECT_EQ(nullable, expected.nullable);
  }

  // 4. Check for data (should be none, but we must check)
  std::cout << "Fetching rows (expecting SQL_NO_DATA)..." << std::endl;
  ret = SQLFetch(conn->hstmt);
  std::cout << "SQLFetch returned: " << ret << std::endl;
  EXPECT_EQ(ret, SQL_NO_DATA);

  EXPECT_EQ(Disconnect(conn), SQL_SUCCESS);

  // Delete table
  std::cout << "Connecting to drop table..." << std::endl;
  EXPECT_EQ(Connect(kDefaultConnectionString, conn), SQL_SUCCESS);
  std::cout << "Dropping table: " << table_name << std::endl;
  table.DropWithPrepare(conn);
  EXPECT_EQ(Disconnect(conn), SQL_SUCCESS);
  std::cout << "--- Finished TEST(StatementTest, SQLDescribeCol_AfterQuery) ---"
            << std::endl;
}
TEST(StatementTest, SQLPrimaryKeys_Metadata) {
  std::cout << "--- Running TEST(StatementTest, SQLPrimaryKeys_Metadata) ---"
            << std::endl;
  std::string const table_name =
      kDatasetWithTablePrefix + "ODBC_SQLPRIMARYKEYS_METADATA_TEST";
  Table table(table_name);
  std::string const schema_name = "INTEGRATION_TESTS";
  ...
  // Connect and call SQLPrimaryKeys
  std::cout << "Connecting to call SQLPrimaryKeys..." << std::endl;
  EXPECT_EQ(Connect(kDefaultConnectionString, conn), SQL_SUCCESS);
  std::cout << "Calling SQLPrimaryKeys for Schema: " << schema_name
            << ", Table: " << table_name << std::endl;
  SQLRETURN ret =
      SQLPrimaryKeys(conn->hstmt, nullptr, 0,  // Use current catalog
                     (SQLCHAR*)schema_name.c_str(), SQL_NTS,
                     (SQLCHAR*)table_name.c_str(), SQL_NTS);
  CheckError(ret, "SQLPrimaryKeys", conn);
  ...
  // 2. Define expected metadata (based on kODBCPrimaryKeysMap)
  struct ExpectedCol {
    std::string name;
    SQLSMALLINT type;
    SQLULEN size;
  ...
  // CORRECTED: Changed SQL_VARCHAR (12) to SQL_WVARCHAR (-9)
  std::vector<ExpectedCol> expected_cols = {
  ...
  // 4. Check for data (should be none)
  std::cout << "Fetching rows (expecting SQL_NO_DATA)..." << std::endl;
  ret = SQLFetch(conn->hstmt);
  std::cout << "SQLFetch returned: " << ret << std::endl;
  EXPECT_EQ(ret, SQL_NO_DATA);
  ...
  table.DropWithPrepare(conn);
  EXPECT_EQ(Disconnect(conn), SQL_SUCCESS);
  std::cout << "--- Finished TEST(StatementTest, SQLPrimaryKeys_Metadata) ---"
            << std::endl;
}
