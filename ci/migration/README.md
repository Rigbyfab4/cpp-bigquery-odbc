...
This repository provides **cross‑platform migration scripts** to move existing BigQuery ODBC DSNs from legacy **Simba drivers** to the new **ODBC Driver for BigQuery**.
...
They follow the **same high‑level behavior and migration semantics** across all platforms, with platform‑specific implementation details.

---

## Goals of the Migration

* Seamlessly transition DSNs from Simba BigQuery drivers to the Google‑provided ODBC Driver for BigQuery
...
4. **Migrates the selected DSN** based on the chosen migration mode (`copy` or `replace`).
5. **Updates registry entries for Windows** so the DSN points to the new Google BigQuery ODBC driver.
...
This unified behavior ensures a **predictable, safe migration experience across Windows, Linux, and macOS**.
