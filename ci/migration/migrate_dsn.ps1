...
foreach ($root in $DSNRoots) {
    ...
    foreach ($entry in $dsns.PSObject.Properties) {
        ...
        if ($driverValue -ne $SimbaDriverName) { continue }
        ...
        # 2. Copy all registry values from Simba DSN
        CopyRegistryTree -SourcePath $srcDSNKey -TargetPath $dstDSNKey

        # 3. Update Driver DLL and TrustedCerts dynamically
        $googleDriverKey = Get-ChildItem "HKLM:\SOFTWARE\ODBC\ODBCINST.INI" |
            Where-Object { $_.PSChildName -eq $GoogleDriverName }
        if (!$googleDriverKey) { Write-Warning "Google driver not found in ODBCINST.INI"; continue }
        ...
        # 4. DECRYPT KeyFilePath_Enc (if present)
